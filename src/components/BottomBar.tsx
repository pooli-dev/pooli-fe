...
const navItems = [
  {
    path: "/support",
    icon: { on: supportOn, off: supportOff },
    label: "Support",
  },
...
export default function BottomBar() {
  ...
  return (
    <nav
      className={`w-full flex justify-around items-center py-2 bg-white/60 backdrop-blur-[10px] pb-[calc(0.5rem+env(safe-area-inset-bottom))] z-[100] ${darkMode ? "invert" : ""}`}
    >
      {navItems.map(({ path, icon, label }) => {
        ...
        return (
          ...
          >
            ...
            />
            <span
              className={`${isActive ? "text-[#0088FF] font-semibold" : "text-[#999999] font-medium"} text-[0.6875rem]`}
            >
              {label}
            </span>
          </button>
