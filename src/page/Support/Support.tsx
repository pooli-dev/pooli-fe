      ...
      const formattedInquiries: Inquiry[] = await Promise.all(
        response.content.map(async (item) => {
          try {
            ...
            const category = categories.find(c => c.questionCategoryId === item.questionCategoryId);
            
            return {
              id: item.questionId,
              categoryId: item.questionCategoryId,
              categoryName: category ? getCategoryDisplayName(category.questionCategoryName) : '기타',
              title: item.title,
              content: detail.content,
              status: item.isAnswer ? '완료' : '대기중',
              date: new Date(detail.createdAt).toLocaleDateString('ko-KR').replace(/\. /g, '.').slice(0, -1),
              response: detail.answer?.content,
              responseDate: detail.answer ? new Date(detail.answer.createdAt).toLocaleDateString('ko-KR').replace(/\. /g, '.').slice(0, -1) : undefined,
  ...
  return (
    <div className="relative h-[calc(100vh-106px-60px)] overflow-y-auto mt-[106px] mb-[60px]">
      <div className="py-5 pb-[60px]">
        ...
        {activeTab === 'inquiry' ? (
          <InquiryForm categories={categories} onSubmitSuccess={handleSubmitSuccess} />
        ) : (
          <InquiryHistory 
            inquiries={inquiries} 
            sortOrder={sortOrder} 
